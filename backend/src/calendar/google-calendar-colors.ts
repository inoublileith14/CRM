/** Google Calendar API color definitions (stable defaults). */
export const GOOGLE_CALENDAR_EVENT_COLORS: Record<
  string,
...
export function resolveGoogleEventColors(
  ...
  if (eventColorId && GOOGLE_CALENDAR_EVENT_COLORS[eventColorId]) {
    const entry = GOOGLE_CALENDAR_EVENT_COLORS[eventColorId];
    return {
  ...
  if (calendarColorId && GOOGLE_CALENDAR_LIST_COLORS[calendarColorId]) {
    const entry = GOOGLE_CALENDAR_LIST_COLORS[calendarColorId];
    return {
      backgroundColor: entry.background,
      foregroundColor: entry.foreground,
    };
  ...
  return {
    backgroundColor: GOOGLE_CALENDAR_EVENT_COLORS['9'].background,
    foregroundColor: GOOGLE_CALENDAR_EVENT_COLORS['9'].foreground,
  };
}
