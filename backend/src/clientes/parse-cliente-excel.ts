...
function parseOrigen(
  value: string,
): 'email' | 'call' | 'otro' | null {
...
export function parseClienteExcelBuffer(
  buffer: Buffer,
  ...
  const colIndex = new Map<keyof BulkImportClienteItemDto, number>();
  ...
  const results: BulkImportClienteItemDto[] = [];

  for (let r = 1; r < rows.length; r++) {
    const row = rows[r];
    if (!row || row.every((c) => !String(c ?? '').trim())) continue;

    const get = (key: keyof BulkImportClienteItemDto) => {
