...
interface InmuebleBcnStatusFilterHeadProps {
  uniqueValues: string[];
...
function formatStatusFilterLabel(value: string): string {
...
export function InmuebleBcnStatusFilterHead({
  uniqueValues,
  filter,
  isOpen,
  isFilterActive,
  ...
  style,
  className = '',
}: InmuebleBcnStatusFilterHeadProps) {
  ...
  const panel =
    isOpen && mounted ? (
      <div
        ref={panelRef}
        role="dialog"
        aria-label="Filtrar por estado BCN"
      ...
      >
        <div className="border-b border-slate-200 px-3 py-2">
          <p className="text-xs font-semibold text-slate-800">Estados</p>
        </div>
        ...
        <div className="max-h-56 overflow-y-auto px-3 py-2">
          ...
          {uniqueValues.map((value) => (
            ...
            >
              ...
              <span
                className={
                  value === BLANK_FILTER_VALUE ? 'italic text-slate-500' : ''
                }
              >
    ...
    <>
      ...
      >
        <button
          ...
          className="flex w-full min-h-[3.25rem] flex-col items-center justify-center gap-1 px-0.5 py-1"
          aria-expanded={isOpen}
          aria-haspopup="dialog"
          title="Filtrar por estado BCN"
        >
          <span className="text-[8px] font-semibold uppercase leading-none text-yellow-300 sm:text-[9px]">
            {formatTableHeaderLabel('BCN')}
          </span>
          <span className="inline-flex items-center justify-center gap-px leading-none">
            <Filter
            ...
            />
            <ChevronDown
              className={`h-2.5 w-2.5 shrink-0 text-white/75 transition sm:h-3 sm:w-3 ${
                isOpen ? 'rotate-180' : ''
              }`}
              aria-hidden
            />
