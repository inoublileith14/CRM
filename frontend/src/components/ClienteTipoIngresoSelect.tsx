...
import {
  ...
  ClienteTipoIngreso,
  getClienteTipoIngresoOption,
  normalizeClienteTipoIngreso,
} from '@/lib/cliente-tipo-ingreso';

interface ClienteTipoIngresoSelectProps {
  clienteId: string;
  value: string | null | undefined;
  disabled?: boolean;
  compact?: boolean;
  onUpdated: (tipoIngreso: ClienteTipoIngreso | null) => void;
}

export function ClienteTipoIngresoSelect({
  clienteId,
  value,
  disabled,
  compact = true,
  onUpdated,
}: ClienteTipoIngresoSelectProps) {
  ...
  const current = getClienteTipoIngresoOption(value);
  const position = useFloatingPanelPosition({
    open,
    triggerRef,
    panelRef,
    minPanelWidth: 160,
    estimatedHeight: CLIENTE_TIPO_INGRESO_OPTIONS.length * 32 + 40,
    deps: [],
  });
  ...
  async function handleSelect(next: ClienteTipoIngreso | null) {
    setOpen(false);
    const currentValue = normalizeClienteTipoIngreso(value);
    if (currentValue === next) return;

    setSaving(true);
    try {
      await updateCliente(clienteId, { tipo_ingreso: next });
      onUpdated(next);
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : 'No se pudo guardar el origen de ingresos',
      );
    } finally {
      setSaving(false);
    }
  ...
  const dropdown =
    open && mounted ? (
      ...
      >
        ...
        {CLIENTE_TIPO_INGRESO_OPTIONS.map((option) => (
          <li
            key={option.value}
            role="option"
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        title={current?.label ?? 'Seleccionar origen de ingresos'}
      >
