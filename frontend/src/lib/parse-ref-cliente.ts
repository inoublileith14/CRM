...
export function buildRefCliente(parts: ParsedRefCliente): string | null {
  const segments: string[] = [];

  if (parts.presupuesto?.trim()) {
    segments.push(parts.presupuesto.trim());
  }
  if (parts.habitaciones != null && Number.isFinite(parts.habitaciones)) {
    segments.push(`${parts.habitaciones}h`);
  }
  if (parts.banos != null && Number.isFinite(parts.banos)) {
    segments.push(`${parts.banos}b`);
  }
  if (parts.metros != null && Number.isFinite(parts.metros)) {
    segments.push(`${parts.metros}m`);
  }
  if (parts.zona?.trim()) {
    segments.push(parts.zona.trim());
  }

  return segments.length > 0 ? segments.join(' ') : null;
}

export function normalizeRefPresupuesto(value: string): string | null {
...
export function setParsedRefField(
  ref: string | null | undefined,
  field: keyof ParsedRefCliente,
  value: string | number | null,
): string | null {
  const parsed = parseRefCliente(ref);
  const next: ParsedRefCliente = {
    ...parsed,
    [field]:
      field === 'presupuesto'
        ? typeof value === 'string'
          ? normalizeRefPresupuesto(value)
          : null
        : value,
  };

  return buildRefCliente(next);
}
